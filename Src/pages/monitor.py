...
class MonitorPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # ... (Top Packet Table and Middle Info Frames remain the same) ...
        ...
        self.anomaly_tree = ttk.Treeview(anomaly_frame, columns=("No", "Time", "DestPort", "Reason", "Score", "Method"), show="headings")
    ...
    def setup_anomaly_treeview(self, tree):
        columns = ("No", "Time", "DestPort", "Reason", "Score", "Method")
        tree["columns"] = columns
        ...
        tree.column("DestPort", width=80, anchor='center')
    ...
    def _perform_gui_update(self, values_for_display, is_anomalous, scapy_packet_idx, flow_details, flow_processed, error_message):
        ...
        try:
            if not flow_processed and values_for_display:
            ...
            if is_anomalous and flow_details:
                ...
                anomaly_display_values = (
                    ...
                    flow_details[4],  # Reason (Predicted Label or Rule Alert)
                    f"{float(flow_details[5]):.2f}",  # Score
                    flow_details[6]   # Detection Method
                )
                self.anomaly_tree.insert("", "end", values=anomaly_display_values)
                self.anomaly_tree.yview_moveto(1.0)
                
        except (tk.TclError, ValueError):
            pass # Window might be closing or value format issue
    ...
    def stop_and_report(self):
        ...
        all_completed_flows = self.packet_capture_manager.all_completed_flows
